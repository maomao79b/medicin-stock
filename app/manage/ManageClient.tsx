...
function ManageClientContent({ initialUnits = [] }: { initialUnits: string[] }) {
  ...
  const getExpiryStatus = (expiryStr: string) => {
    if (!expiryStr) return { label: '🟢', color: 'var(--success)' };
    ...
    if (diffDays < 0) return { label: '⚫', color: '#666' };
    if (diffDays < 90) return { label: '🔴', color: 'var(--danger)' };
    if (diffDays <= 180) return { label: '🟡', color: 'var(--warning)' };
      ...
      <div className="page-enter">
        ...
        {scannerMode && (
          <div className="card mb-4 page-enter">
            ...
            <BarcodeScanner onScan={async (text: string) => {
